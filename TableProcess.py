...
class TableProcessModel:
    def __init__(self) -> None:
        ...
        self.table_structure_feature_extractor_model = (
            ModelManager.get_table_structure_feature_extractor_model()
        )
        self.table_structure_split_model = (
            ModelManager.get_table_structure_split_model()
        )
        self.table_wired_structure_split_model = (
            ModelManager.get_table_wired_structure_split_model()
        )
    ...
    def infer_locate_table(self):  # ~2s
        ...
        img = cv2.imdecode(
            np.fromfile(self.image_path, dtype=np.uint8), cv2.IMREAD_COLOR
        )
        if img is None:
    ...
    def infer_split(self, encoding, target_sizes):
        ...
        results = (
            self.table_structure_feature_extractor_model.post_process_object_detection(
    ...
    def parse_table_split_result(self):
        ...
        """
        self.cols_box_list = [
            self.table_split_result["boxes"][i].tolist()
            for i in range(len(self.table_split_result["boxes"]))
            if self.table_split_result["labels"][i].item() == 1
        ]

        self.rows_box_list = [
            self.table_split_result["boxes"][i].tolist()
            for i in range(len(self.table_split_result["boxes"]))
            if self.table_split_result["labels"][i].item() == 2
        ]
    ...
    def cell_match_table(self):
        ...
        self.cells_box_list = [
            TableProcessModel.intersection(row, col)
    ...
    @timeit_decorator(enable_print=False)
    def draw_boxs(
            self, image: Image, draw_col=True, draw_row=True, draw_cell=True, cut_cell=False
    ):
        ...
        if draw_cell and (len(self.cells_box_list) > 0):
            ...
            white_background.save(self.each_image_mid_dir + "/" + "sorted_cells.jpg")
        if cut_cell and (len(self.cells_box_list) > 0):
            ...
            for i, cell in enumerate(self.cells_box_list):
                ...
                cell_image.save(
                    self.each_image_mid_dir + "/" + f"cell_{x}_{y}" + ".jpg"
                )
